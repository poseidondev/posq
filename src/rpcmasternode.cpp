...
Value obfuscation(const Array& params, bool fHelp)
{
    ...
    if (fHelp || params.size() == 0)
        throw runtime_error(
            "obfuscation <posqaddress> <amount>\n"
            "posqaddress, reset, or auto (AutoDenominate)"
    ...
    if (params.size() != 2)
        throw runtime_error(
            "obfuscation <posqaddress> <amount>\n"
            "posqaddress, denominate, or auto (AutoDenominate)"
            "<amount> is a real and will be rounded to the next 0.1" +
            HelpRequiringPassphrase());
...
Value masternodeconnect(const Array& params, bool fHelp)
{
    if (fHelp || (params.size() != 1))
        throw runtime_error(
            ...
            "\nExamples:\n" +
            HelpExampleCli("masternodeconnect", "\"192.168.0.6:5510\"") + HelpExampleRpc("masternodeconnect", "\"192.168.0.6:5520\""));
...
Value masternodecurrent (const Array& params, bool fHelp)
{
    if (fHelp || (params.size() != 0))
        throw runtime_error(
            ...
            "  \"lastseen\": posq,       (numeric) Time since epoch of last seen\n"
            "  \"activeseconds\": posq,  (numeric) Seconds MN has been active\n"
            "}\n"
            "\nExamples:\n" +
