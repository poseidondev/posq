...
std::string HelpMessageCli()
{
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Connect to JSON-RPC on <port> (default: %u or testnet: %u)"), 15520, 38843));
...
static bool AppInitRPC(int argc, char* argv[])
{
    ...
    if (argc < 2 || mapArgs.count("-?") || mapArgs.count("-help") || mapArgs.count("-version")) {
        std::string strUsage = _("Posq Core RPC client version") + " " + FormatFullVersion() + "\n";
        if (!mapArgs.count("-version")) {
            strUsage += "\n" + _("Usage:") + "\n" +
                        "  posq-cli [options] <command> [params]  " + _("Send command to Posq Core") + "\n" +
                        "  posq-cli [options] help                " + _("List commands") + "\n" +
                        "  posq-cli [options] help <command>      " + _("Get help for a command") + "\n";
