...
Clone the git repositories for posq and gitian and then checkout the posq version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the posq repository
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/posq/.git/
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/posq-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/posq-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/posq-build.assert
```
